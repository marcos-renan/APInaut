    ...
    <section className="flex min-h-0 flex-col overflow-hidden border-y border-white/10 bg-[#1a1728] px-0 py-3">
      <div className="mb-2 grid shrink-0 grid-cols-3 gap-1 px-3">
        <div className="rounded-md border border-white/10 bg-[#121025] px-2 py-1.5">
          <p className="text-[10px] uppercase tracking-wide text-zinc-400">{t("response.status")}</p>
          <p
        ...
        <div className="rounded-md border border-white/10 bg-[#121025] px-2 py-1.5">
          <p className="text-[10px] uppercase tracking-wide text-zinc-400">{t("response.time")}</p>
          <p className="mt-0.5 text-xs font-semibold text-zinc-100">{secondsDisplay}</p>
        </div>
        <div className="rounded-md border border-white/10 bg-[#121025] px-2 py-1.5">
          <p className="text-[10px] uppercase tracking-wide text-zinc-400">{t("response.transferred")}</p>
          ...
          {responseTab === "body" && responseBodyView === "web" ? (
            <div className="h-full min-h-0 overflow-hidden rounded-none border-0 bg-[#0f0d18]">
              {requestError ? (
                ...
                <div className="p-3 text-sm text-zinc-400">{t("response.none")}</div>
              ) : (
                <iframe
            ...
            <CodeEditor
              ...
              placeholder={
                responseTab === "cookies"
                  ? t("response.noneCookies")
                  : responseTab === "headers"
                    ? t("response.noneHeaders")
                    : t("response.none")
              }
            />
          )}
